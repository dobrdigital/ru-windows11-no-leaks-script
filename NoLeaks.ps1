...
# NoLeaks TUI v3.0 — Single-file, Cyrillic, arrow-key navigation
...
# ============================================================
# STRINGS — proper Cyrillic via UTF-8
# ============================================================
$script:STR = @{
    EN = @{
        ...
        admin_warn = "Run as Administrator!"
        ...
        p5_name = "Telemetry and Services"
        ...
        selected = "SELECTED"
        phase_hint = "Press ENTER to run this phase"
    }
    RU = @{
        title = "Windows 11 No-Leaks"
        subtitle = "Защита от утечек IP"
        version = "v3.0"
        lang_select = "Выберите язык:"
        menu_header = "ГЛАВНОЕ МЕНЮ"
        menu_run_all = "Запустить все фазы"
        ...
        admin_warn = "Запустите от имени Администратора!"
        admin_ok = "Администратор: ДА"
        admin_no = "Администратор: НЕТ (ограничено)"
        settings = "НАСТРОЙКИ"
        current = "Текущее"
        enter_tun = "Имя TUN-адаптера"
        enter_mtu = "Значение MTU"
        enter_ip = "Ожидаемый IP (необязательно)"
        saved = "Настройки сохранены!"
        confirm_all = "Запустить ВСЕ фазы?"
        yes_no = " (1=Да / 2=Нет)"
        summary = "ИТОГИ"
        total_pass = "Всего PASS"
        total_fail = "Всего FAIL"
        total_warn = "Всего WARN"
        reboot_now = "Перезагрузить сейчас?"
        reboot_1 = "1. Да, перезагрузить"
        reboot_2 = "2. Нет, позже"
        press_key = "Нажмите любую клавишу..."
        p1_name = "Блокировка WebRTC/STUN/TURN"
        p1_desc = "Правила файрвола, LLMNR/NBT-NS off"
        p2_name = "Отключение QUIC"
        p2_desc = "msquic, HTTP/3, политики браузеров"
        p3_name = "Отключение IPv6"
        p3_desc = "Биндинги, туннели, DisabledComponents"
        p4_name = "Оптимизация TCP стека"
        p4_desc = "TTL=128, Fast Open/ECN off, Nagle off"
        p5_name = "Телеметрия и службы"
        p5_desc = "DiagTrack, SSDP, mDNS, блокировка hosts"
        p6_name = "Watcher MTU"
        p6_desc = "Поддерживает MTU + IPv6 off на TUN"
        p7_name = "Финальный аудит"
        p7_desc = "17 категорий проверок утечек"
        nav_hint = "ВВЕРХ/ВНИЗ = выбор | ENTER = запуск | A = Все | S = Настройки | Q = Выход"
        selected = "ВЫБРАНО"
        phase_hint = "Нажмите ENTER для запуска этой фазы"
    }
}
...
$script:MenuIndex = 0  # 0-6 = phases, 7 = Run All, 8 = Settings, 9 = Quit
...
function Draw-Screen {
    Clear-Host
    $w = [Math]::Min($Host.UI.RawUI.WindowSize.Width - 1, 78)

    # Header
    Write-Host ""
    $header = "  " + (STR "title") + " " + (STR "version")
    ...
    # Admin status
    if (Is-Admin) {
        Write-Host ("  " + (STR "admin_ok")) -ForegroundColor Green
    } else {
        Write-Host ("  " + (STR "admin_no")) -ForegroundColor Yellow
    }
    Write-Host ("  TUN: " + $script:CFG.TunName + " | MTU: " + $script:CFG.TunMtu) -ForegroundColor DarkGray
    Write-Host ""

    # Phase list with arrow cursor
    ...
    for ($i = 0; $i -lt 7; $i++) {
        ...
        $bgColor = if ($script:MenuIndex -eq $i) { "Blue" } else { "Black" }
        $fgColor = if ($script:MenuIndex -eq $i) { "White" } else { "Gray" }
        ...
        if ($script:MenuIndex -eq $i) {
            ...
            Write-Host ("  {0}{1}. {2,-28} " -f $cursor, $num, $pName) -NoNewline -ForegroundColor Gray
            Write-Host ("{0,-28} " -f $pDesc) -NoNewline -ForegroundColor DarkGray
    ...
    # Run All
    $cursor = if ($script:MenuIndex -eq 7) { ">" } else { " " }
    if ($script:MenuIndex -eq 7) {
        Write-Host ("  {0} [A] {1}" -f $cursor, (STR "menu_run_all")) -ForegroundColor Cyan -BackgroundColor DarkBlue
    } else {
        Write-Host ("  {0} [A] {1}" -f $cursor, (STR "menu_run_all")) -ForegroundColor White
    }

    # Settings
    $cursor = if ($script:MenuIndex -eq 8) { ">" } else { " " }
    if ($script:MenuIndex -eq 8) {
        Write-Host ("  {0} [S] {1}" -f $cursor, (STR "menu_settings")) -ForegroundColor Cyan -BackgroundColor DarkBlue
    } else {
        Write-Host ("  {0} [S] {1}" -f $cursor, (STR "menu_settings")) -ForegroundColor White
    }

    # Quit
    $cursor = if ($script:MenuIndex -eq 9) { ">" } else { " " }
    if ($script:MenuIndex -eq 9) {
        Write-Host ("  {0} [Q] {1}" -f $cursor, (STR "menu_quit")) -ForegroundColor Cyan -BackgroundColor DarkBlue
    } else {
        Write-Host ("  {0} [Q] {1}" -f $cursor, (STR "menu_quit")) -ForegroundColor White
    }
    ...
    if ($script:MenuIndex -lt 7) {
        ...
        Write-Host ("     Runs all 7 phases sequentially") -ForegroundColor Gray
    } elseif ($script:MenuIndex -eq 8) {
        Write-Host ("  >> " + (STR "selected") + ": " + (STR "menu_settings")) -ForegroundColor Yellow
        Write-Host ("     Configure TUN name, MTU, expected IP") -ForegroundColor Gray
    } else {
    ...
    switch ($Phase) {
        ...
        5 {
            ...
            foreach ($svcName in $services) {
                ...
                if ($svc) { Stop-Service $svcName -Force -ErrorAction SilentlyContinue; Set-Service $svcName -StartupType Disabled -ErrorAction SilentlyContinue }
            ...
            foreach ($domain in $telemetryDomains) {
                if ($existingHosts -notcontains "0.0.0.0 $domain") { Add-Content -Path $hostsPath -Value "0.0.0.0 $domain" -ErrorAction SilentlyContinue; $added++ }
            }
...
# ============================================================
# MAIN LOOP — Arrow key navigation
# ============================================================
Show-LanguageSelect
...
while ($true) {
    ...
    switch ($key.VirtualKeyCode) {
        38 { # UP arrow
            if ($script:MenuIndex -gt 0) { $script:MenuIndex-- }
        }
        40 { # DOWN arrow
            if ($script:MenuIndex -lt 9) { $script:MenuIndex++ }
        }
        13 { # ENTER
            if ($script:MenuIndex -lt 7) {
                Run-PhaseWithUI -PhaseNum ($script:MenuIndex + 1)
            } elseif ($script:MenuIndex -eq 7) {
                Run-AllPhasesWithUI
            } elseif ($script:MenuIndex -eq 8) {
                Show-Settings
